...
ALTER TABLE movies
ALTER COLUMN age_certificate TYPE VARCHAR(30);

INSERT INTO movies (movie_name, movie_length, movie_lang, release_date,
age_certificate, director_id) 

VALUES ('A Clockwork
Orange','112','English','1972-02-02','18','13'),
...
/*
SELECT  columnname, columan2... 
FROM TABLE 1
JOIN 
TABLE 2
ON TABLE1.director_id = TABLE2.dirctor_id
*/
...
VALUES ('Christopher','Nolan', date'1970-07-30', 'British');
...
SELECT m.movie_name, r.domestic_takings 
FROM movies m 
JOIN movie_revenues r
--ON m.movie_id=r.movie_id
USING (movie_id)
WHERE m.age_certificate IN ('12','18','10')
...
FROM directors d
JOIN movies m
ON d.directors_id=m.directors-id
WHERE m.movie_lang IN ( , , )
...
SELECT m.movie_name, mr.international_takings, mr.domestic_takings 
FROM moive_revenues mr 
JOIN moives m
ON mr.....

WHERE mr.domestic_takings IS NULL
OR mr.internatinal_takings IS NULL
ORDER BY m.movie_name

	--1--
SELECT*FROM directors

SELECT directors.first_name, directors.last_name, movie.movie_name, movie.release_date
FROM directors
JOIN movie
ON directors.director_id=movie.director_id
WHERE movie.movie_lang = 'Chinese' or movie.movie_lang = 'Korean' or movie.movie_lang = 'Japanese'
	
	--2--
SELECT movie.movie_name, movie.release_date, movie_revenues.international_takings
FROM movie
JOIN movie_revenues
ON movie.movie_id=movie_revenues.movie_id
WHERE movie_lang = 'English'

	--3--
SELECT movie.movie_name, movie_revenues.domestic_takings, movie_revenues.domestic_takings
FROM movie
JOIN movie_revenues
ON movie.movie_id=movie_revenues.movie_id
...
SELECT t1.coumnname1, t1.column2, t2.column1
FROM table1 t1
LEFT JOIN
table2 t2
...
SELECT d.director_id, d.first_name, d.last_name, m.movie_name 
FROM 
directors d RIGHT JOIN movies m
...
ON d.director_id =m.director_id
WHERE m.movie_lang IN ('German','Korean')
...
LEFT JOIN movie m
ON d.director_id=m.director_id
GROUP BY d.first_name, d.last_name, m.movie_id
...
SELECT * FROM actors
SELECT a.first_name,a.last_name,m.movie_name,mr.international_takings,mr.domestic_takings
FROM directors d
...
--Assignment/Practice--
--1--
SELECT d.first_name,d.last_name,a.first_name,a.last_name
FROM directors d
JOIN actors a ON d.director_id = a.director_id
WHERE d.first_name='Wes' AND d.lat_name='Anderson'
...
WHERE date_of_birth BETWEEN '1960-01-01' AND '1970-01-01'
UNION 
...
--find out the director older than director James Cameron
SELECT first_name, last_name, date_of_birth FROM directors
WHERE date_of_birth > (SELECT date_of_birth FROM directors
					  WHERE first_name='James' 
					  AND last_name ='Cameron')
					  
--Uncorelated Subqueries vs corelated Subqueries--
--check all the movies (parent) sells better internationally has higher international takings than domestic takings (child)--
...
WHERE movie_length = (SELECT AVG(age_certificates) FROM movies 
					 WHERE age_certificates = age_certificates)
